...
function onClickUserEditForm(count) {
  let dbuser = localStorage.getItem(NAME_DB)
  if (dbuser) {
    dbuser = JSON.parse(dbuser)
    const userEdit = dbuser.users[count]
    console.log(userEdit)
    const dniEdit = document.getElementById('dni-edit')
    const userNameEdit = document.getElementById('username-edit')
    const surnameEdit = document.getElementById('surname-edit')
    const dogRazaEdit = document.getElementById('dog-edit')
    dniEdit.value = userEdit.dni
    userNameEdit.value = userEdit.username
    surnameEdit.value = userEdit.surname
    dogRazaEdit.value = userEdit.dog
  }
}
  ...
  while (count < db.users.length) {
    const template = `
        <tr>
            ...
            <td>${db.users[count].dni}</td>
            <td>${db.users[count].name}</td>
            <td>${db.users[count].surname}</td>
            <td>${db.users[count].dog}</td>
            <td>
                <!-- button group -->
                <div class="btn-group" role="group" aria-label="Basic mixed styles example">
                    <button onclick="onClickUserEditForm(${
                      db.users[count].id
                    })" type="button" class="btn btn-success"
                    data-toggle="modal"
                    data-target="#exampleModal-edit"
                    >Edit</button>
                    <button onclick="onUserRomove(${
                      db.users[count].id
